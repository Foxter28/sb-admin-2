...
function tambah_user($data)
{
    ...
    // Gunakan strtolower dan stripslashes untuk sanitasi tambahan pada username
    $username  = strtolower(stripslashes($data["username"]));
    $password  = $data["password"]; // Tidak perlu htmlspecialchars di sini, karena akan di-hash
    $user_role = htmlspecialchars($data["user_role"]);

    // --- START PERBAIKAN ---

    // 1. Cek apakah ID USER sudah ada (untuk mengatasi error 'u01')
    $check_id = mysqli_query($koneksi, "SELECT id_user FROM users WHERE id_user = '$kode'");
    if (mysqli_fetch_assoc($check_id)) {
        echo "<script>alert('ID User ($kode) sudah terdaftar. Gunakan ID lain.');</script>";
        return false;
    }
    
    // 2. Cek apakah USERNAME sudah ada (praktik keamanan & UX yang baik)
    $check_username = mysqli_query($koneksi, "SELECT username FROM users WHERE username = '$username'");
    ...
    mysqli_query($koneksi, $query); // Baris 111 yang error sebelumnya

    return mysqli_affected_rows($koneksi);
}

// ... (fungsi-fungsi user lainnya) ...
